...
# Add 2 colums to XTest: subject and activity
XTest <- cbind(SubjectTest,yTest, XTest)
names(XTest)[1:2] <- c("subject","activity")
for(i in 1:6){
        XTest[,2] <- gsub(ActivityLabels[i,1],ActivityLabels[i,2],XTest[,2])
}

# Add 2 colums to XTrain: subject and activity
XTrain <- cbind(SubjectTrain, yTrain, XTrain)
names(XTrain)[1:2] <- c("subject","activity")
for(i in 1:6){
...
# Make sure the first 2 colums are considered factors
...
for(i in 1:nrow(df)){
        s <- df$subject[i]
        a <- df$activity[i]
        df[i,3:88]  <- colMeans(HumanActivity[HumanActivity$subject == s & HumanActivity$activity == a,3:88])
}
